...
impl AutoDreamService {
    ...
    async fn count_recent_sessions(&self, since: &DateTime<Utc>) -> anyhow::Result<usize> {
        ...
        let sessions_dir = home.join(".rustcode").join("sessions");
    ...
    async fn try_acquire_lock(
        ...
        let home = dirs::home_dir().unwrap_or_else(|| PathBuf::from("."));
        let lock_path = home.join(".rustcode").join(".consolidation.lock");
    ...
    async fn load_memories(&self) -> anyhow::Result<Vec<MemoryEntry>> {
        let home = dirs::home_dir().unwrap_or_else(|| PathBuf::from("."));
        let memory_path = home.join(".rustcode").join("memory.json");
    ...
    async fn save_consolidated_memories(
        ...
        let home = dirs::home_dir().unwrap_or_else(|| PathBuf::from("."));
        let consolidated_path = home.join(".rustcode").join("consolidated_memories.json");
    ...
    async fn save_state(&self, state: &ConsolidationState) -> anyhow::Result<()> {
        let home = dirs::home_dir().unwrap_or_else(|| PathBuf::from("."));
        let state_path = home.join(".rustcode").join(".autodream_state.json");
